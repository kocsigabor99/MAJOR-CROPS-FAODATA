...
def generate_optimized_meal_plan(daily_needs_per_citizen, food_data, max_foods, max_attempts, population, total_needs):
    ...
    final_scaled_plan = {
        food_type: [
            {**item, "Total (kg)": item["Grams"] * population * 365} for item in items
...
# Function to compare final scaled plan with annual nutrient needs
def compare_scaled_plan_with_annual_needs(final_scaled_plan, nutrient_needs_df, country, year):
    # Get the annual nutrient needs for the selected country and year (multiplied by 365)
    annual_nutrient_needs = nutrient_needs_df[(nutrient_needs_df['Region, subregion, country or area'] == country) & 
                                               (nutrient_needs_df['Year'] == year)]
    
    # Multiply the daily needs by 365 for annual needs
    annual_nutrient_needs_scaled = annual_nutrient_needs.copy()
    numeric_columns = annual_nutrient_needs_scaled.select_dtypes(include=['float64', 'int64']).columns
    annual_nutrient_needs_scaled[numeric_columns] *= 365
    
    # Extract total nutrients from the final scaled plan (summed up across all food types)
    final_nutrients = {nutrient: 0 for nutrient in annual_nutrient_needs_scaled.columns[2:]}
    for food_type, items in final_scaled_plan.items():
        for item in items:
            for nutrient in final_nutrients.keys():
                final_nutrients[nutrient] += clean_nutrient_value(item.get(nutrient, 0)) * (item["Total (kg)"] * 1000 / 100.0)

    # Calculate the percentage of each nutrient met by the final scaled plan
    percentage_met = {}
    for nutrient, total_value in final_nutrients.items():
        if nutrient in annual_nutrient_needs_scaled.columns:
            required_value = annual_nutrient_needs_scaled[nutrient].values[0]
            if required_value > 0:
                percentage_met[nutrient] = (total_value / required_value) * 100

    return percentage_met, final_nutrients, annual_nutrient_needs_scaled


# Streamlit UI to generate and display results
if st.button("Generate Country-Scale Meal Plan"):
    # Generate the optimized meal plan and get the best iteration
    ...
    if best_iteration:
        ...
        # Comparison between the final scaled plan and the annual nutrient needs (multiplied by 365)
        # Function to compare final scaled plan with annual nutrient needs
        percentage_met, final_nutrients, annual_nutrient_needs_scaled = compare_scaled_plan_with_annual_needs(
            final_scaled_plan, nutrient_needs_df, country, year
        )

        # Display the comparison results
        st.subheader("Comparison of Final Scaled Plan with Annual Nutrient Needs")
        st.write("Total Nutrients in Final Scaled Plan (kg):", final_nutrients)
        st.write("Annual Nutrient Needs (from data, scaled to 365 days):", annual_nutrient_needs_scaled[nutrient_needs_df.columns[2:]])
        st.write("Percentage of Nutrient Needs Met by Final Scaled Plan:", percentage_met)
